...
function showGroupLayout()
{
	...
	xmlhttp = getXmlHttp();
	var selectedGroup = cleanInput($('#formGroup select:eq(0)').val());
	xmlhttp.open('GET', scriptIndexPath + 'action=layout&group=' + selectedGroup + '&rand=' + Date(), true);
	xmlhttp.onreadystatechange = function()
	{
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200)
		{
...
function loadIframe()
{
	...
	var selectedGroup = cleanInput($('#formGroup select:eq(0)').val());
	xmlhttp = getXmlHttp();
	xmlhttp.open('GET', scriptIndexPath + 'action=iframe&group=' + selectedGroup + '&rand=' + Date(), true);
	xmlhttp.onreadystatechange = function()
	{
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200)
		{
...
function saveGroupLayout()
{
	...
	xmlhttp = getXmlHttp();
	xmlhttp.open('GET', scriptIndexPath + 'action=layoutupdate&group=' + selectedGroup + '&url=' + encodeURIComponent(document.getElementById('formUrl').value) + '&left=' + document.getElementById('layout-left-' + i).value + '&right=' + document.getElementById('layout-right-' + i).value + '&center=' + document.getElementById('layout-center-' + i).value + '&rand=' + Date(), true);
	xmlhttp.onreadystatechange = function()
	{
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200)
		{
